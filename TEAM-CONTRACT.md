...
> 本文件为 Claude Code 消费而写，非人类文档。假设读者是另一个 CC 会话，正在接手 B/D/E/F 中一个板块，需要与 A+C（本仓库负责的板块）无缝集成。
>
...
- **Demo**: 公网可访问，用户自有服务器
- **本仓库所有者板块**: A（用户/认证）+ C（活动锅），前端 + 后端全包
- **队友板块**（未分配具体人员）: B（冰箱/食材/拍照识图）/ D（匹配）/ E（聊天）/ F（信用）
...
-- 邀请码：每用户 1 张"码"，但允许消费 3 次（Figma frame-02/04 确认"每人限量 3 个邀请名额"）
CREATE TABLE u_invite_codes (
  id              UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  owner_id        UUID NOT NULL REFERENCES u_users(id) ON DELETE CASCADE,
  code            VARCHAR(10) UNIQUE NOT NULL,       -- 格式 "LINSH-XXXX"，固定前缀 + 4 字符
...
CREATE INDEX ON u_invite_redemptions (code_id);
CREATE UNIQUE INDEX ON u_invite_redemptions (redeemed_by);  -- 每用户只能用一次注册
...
| POST | `/me/avatar` | ✓ | `multipart/form-data` (field: `file`，≤ 2 MB，MIME ∈ `image/jpeg` / `image/png` / `image/webp`) | `{ avatarUrl: string }` | `USER_AVATAR_TOO_LARGE`, `USER_AVATAR_BAD_FORMAT` |
...
- 格式：`LINSH-XXXX`（固定前缀 + 4 字符 Crockford Base32，去掉易混字符 I/L/O/U），示例 `LINSH-7823`
- 冲突时重试（UNIQUE 约束 + INSERT ... ON CONFLICT DO NOTHING + 重试 5 次）
- 每用户恰好 1 张 code，注册成功即发，**可被 3 人消费**（`max_uses=3`、`uses_remaining` 递减）
- 每消费 1 次：
  - `u_invite_redemptions` 插入一条记录（含 `redeemed_by`）
  - `u_invite_codes.uses_remaining` -1（CAS：`WHERE uses_remaining > 0`）
- `GET /me/invite-code` 返回 `{ code, maxUses, usesRemaining, redemptions: [{ userId, nickname, redeemedAt }] }`，前端展示"每人限额 3 次邀请·已用 N 次"
- 一个用户只能被邀请一次（`u_invite_redemptions.redeemed_by` UNIQUE）

### §5.7 徽章（冰箱贴）预置数据
Figma frame-01/13 显示 4 种冰箱贴，以 Figma 中文名为准、code 用英文：

`apps/api/migrations/<timestamp>-seed-badges.ts`：
```sql
INSERT INTO u_badges (code, name, description, icon_url) VALUES
  ('taste_master',    '口味达人',   '尝试 5 种以上不同菜系',        '/uploads/badges/taste-master.svg'),
  ('healthy_life',    '健康生活',   '连续 7 天记录健康食材',         '/uploads/badges/healthy-life.svg'),
  ('warm_host',       '热情房主',   '作为锅主成功主持 3 场活动',     '/uploads/badges/warm-host.svg'),
  ('punctual_diner',  '准时达人',   '10 次活动 100% 准时，无爽约', '/uploads/badges/punctual-diner.svg');
```

MVP 阶段徽章颁发由 F（信用）或后续统计任务触发；A 只提供：
- `GET /badges` — 所有徽章定义（4 条）
- `GET /me/badges` — 当前用户已获得的徽章 code 列表
- `GET /users/:id/public.badges` — 在 UserPublic 响应里返回 code 数组
...
-- B 未就绪时的 fallback：允许用户起锅时手动输入食材名
...
export const CreateActivityRequestSchema = z.object({
  ...
  manualIngredients: z.array(z.string().min(1).max(50)).max(20).default([]),  // B 未就绪 fallback
});
```
...
export const ListActivitiesQuerySchema = PaginationQuerySchema.extend({
  ...
  scope: z.enum(['mine', 'all']).default('all'),       // 'mine' = 我发起或参与的
...
### §6.7 活动消息中心（Figma frame-07..10 "这几锅饭"）

Figma 里的"这几锅饭"是一个**活动维度**的消息汇总页，按活动状态分 tab。
本仓库承担**聚合读取**逻辑；消息内容来自：
- 聊天最后一条消息（来自 E 板块 `ChatClient.getLastMessage(roomId)`）
- 系统通知（来自 `activity.*` 事件，由 C 自己产生的文案）

**端点**：`GET /activity-feeds?status=<filter>&page=&pageSize=`

- `status` 参数枚举：`ALL | UNREAD | IN_PROGRESS | STARTING_SOON`
- 每条 feed item 形如：
  ```ts
  {
    activityId: uuid,
    activityTitle: string,
    activityEmoji: string,          // 从活动标题中生成或默认锅 emoji
    activityStatus: ActivityStatus,
    lastMessage: {
      type: 'CHAT' | 'SYSTEM',
      preview: string,              // 前 30 字
      senderNickname: string | null,// 系统消息为 null
      unreadCount: number,          // 基于用户已读位点
      at: string (datetime),
    } | null,
...
- `POST /activity-feeds/:activityId/read` — 标记已读（前端进入活动详情或聊天室时调用）
- 未读数 = COUNT(messages WHERE created_at > last_read_at)，通过 E 的 `ChatClient.countUnread(roomId, since)` 取（E 未就绪时 mock 0）

**Tab 过滤逻辑**：
| Tab | 过滤条件 |
|---|---|
| 全部 | 当前用户是发起者或参与者的所有活动（含 COMPLETED 最近 7 天） |
| 未读 | 上面集合中 `unreadCount > 0` |
| 进行中 | `activityStatus IN ('FORMED', 'IN_PROGRESS')` |
| 快开始 | `activityStatus = 'STARTING_SOON'`，或 FORMED 且 `startTime - NOW < 2h` |
...
| A 板块 spec → 实现 → 验收 | ⏳ 本仓库主线 |
| C 板块 spec → 实现 → 验收 | ⏳ 本仓库主线 |
| B/D/E/F 实现 | ⏳ 队友主线 |
| 联调 + Playwright | ⏳ Day 3 |
| 部署公网 demo | ⏳ Day 3 末 |
